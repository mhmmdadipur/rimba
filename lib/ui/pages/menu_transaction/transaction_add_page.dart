...
class _TransactionAddPageState extends State<TransactionAddPage> {
  ...
  void initState() {
    ...
    _invoiceController.text =
        'INV-${DateTime.now().millisecondsSinceEpoch}-${_userController.user.value?['id']}';
    _products(_cartController.selectedProducts.deepCopy());
  ...
  Future<void> onTapSubmitButton() async {
    ...
    if (res != null) {
      _isLoading(true);
      await _transactionController.createTransaction(body: dataForm);
      _isLoading(false);
    }
  }
