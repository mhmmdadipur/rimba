...
class _TransactionPageState extends State<TransactionPage> {
  ...
  void initState() {
    ...
    if (_transactionController.listTransaction.isRxNull) {
      _transactionController.getListTransaction().then((value) => getRange());
    } else {
      getRange();
    }
  }
  ...
  void onSearch(String text) {
    if (text.isNotEmpty) {
      _dataView(_transactionController.listTransaction.value?.where((element) {
  ...
  void onLoading() {
    int end;
    List? listTransaction = _transactionController.listTransaction.value;
    int totalDifference =
        listTransaction?.length ?? 0 - (_page * _totalOnceDisplayed);
    ...
    List? temp =
        listTransaction?.getRange(_dataView.value!.length, end).toList();
    if (temp != null) {
  ...
  void getRange() {
    List? listTransaction = _transactionController.listTransaction.value;
    if ((listTransaction?.length ?? 0) > _totalOnceDisplayed) {
      _dataView(
          listTransaction?.getRange(0, _page * _totalOnceDisplayed).toList());
    } else {
      _dataView(listTransaction?.getRange(0, listTransaction.length).toList());
    }
    _page += 1;
  }
          ...
          () => _dataView.isRxNull
              ...
              : SmartRefresher(
                  ...
                  enablePullUp: _dataView.value!.length <
                      (_transactionController.listTransaction.value?.length ??
                          0),
  ...
  Widget renderCard(Map data) {
    return InkWell(
      onTap: () async {
        FocusManager.instance.primaryFocus?.unfocus();
        await Get.to(() => const TransactionDetailPage(),
            arguments: '${data['id']}');
        _page -= 1;
        getRange();
      },
